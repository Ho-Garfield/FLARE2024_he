...
if __name__ =="__main__":
    ...
    model.setup(opt)
    file_paths = [os.path.join(input_dir,f) for f in os.listdir(input_dir) if f.endswith(".nii")]
    import shutil
    for fp in file_paths:
        out_path = os.path.join(out_dir,str.replace(os.path.basename(fp),".nii","_0000.nii.gz"))
        print(out_path)
