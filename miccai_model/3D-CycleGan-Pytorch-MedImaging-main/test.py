...
if __name__ == '__main__':
    ...
    inference_dir = "/DATA_16T/MICCAI/sure/Flip_resample_pre"
    out_dir ="/DATA_16T/MICCAI/sure/Flip_resample_pre_ct_300"
    os.makedirs(out_dir,exist_ok=True)
    model = create_model(opt)
    model.setup(opt)
    file_paths = [os.path.join(inference_dir,f) for f in os.listdir(inference_dir) if f.endswith(".nii.gz")]
    for fp in file_paths:
        out_path = os.path.join(out_dir,os.path.basename(fp))
        print(out_path)
        inference(model, fp, out_path, opt.resample, opt.new_resolution, opt.patch_size[0],
